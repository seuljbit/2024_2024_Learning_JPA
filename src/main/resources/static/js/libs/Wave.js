var Wave = Wave || ( function () {
    ...
    function build() {
        ...
        for (i = 0; i < TOTAL_WAVE; i++) {
            ...
            for (k = 0; k < TOTAL_POINTS; k++) {
                // 더 큰 초기 진폭 설정
                point = {
                    x: _maxWidth / (TOTAL_POINTS - 1) * k - _widthGap,
                    y: _stageHeightHalf + (Math.random() * 300) - 150, // 초기 진폭 값을 두 배로 증가
                    vy: 0
                };
                _points[i][k] = point;
            }
    ...
    function start() {
        ...
        // 클릭 이벤트로 물결 리셋
        $canvas.addEventListener('click', resetWave);
    ...
    // 물결 리셋 함수 추가
    function resetWave() {
        _points.forEach(wave => {
            wave.forEach(point => {
                // 큰 초기 진폭으로 재설정
                point.y = _stageHeightHalf + (Math.random() * 150) - 75;
                point.vy = 0; // 속도 초기화
            });
        });
        dampingFactor = 0.95; // 감쇠 계수 초기화
    }
    ...
    function loop() {
        ...
        for (i=0; i<TOTAL_WAVE; i++) {
            // 감쇠 계수를 적용해 진폭을 줄임
            _points[i].forEach(point => {
                point.y *= dampingFactor;
            });
        ...
        // 점점 감쇠 계수를 줄여가며 물결을 잔잔하게 만듦
        if (dampingFactor > 0.98) dampingFactor *= 0.99;
    }
